...
5. Tolerance and Fallback (Crucial!):
   - Be HIGHLY tolerant of typos, bad grammar, and vague questions (e.g., "what data do we have?", "what is datawaarehouse").
   - If the query has typos, correct them silently in the rewritten question.
   - If the query is broad or open-ended, rewrite it into a general exploratory search query.
   - ONLY mark as unclear if the query is complete gibberish or absolute nonsense. Do not reject queries just because they are broad or have typos.
Input:
